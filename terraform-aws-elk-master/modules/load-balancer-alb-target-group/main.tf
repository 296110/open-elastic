...
resource "aws_alb_listener_rule" "http_path" {
  count = var.num_http_listener_arns

  listener_arn = var.http_listener_arns[count.index]
...
resource "aws_alb_listener_rule" "https_path" {
  count = var.num_https_listener_arns

  listener_arn = var.https_listener_arns[count.index]
  priority     = var.listener_rule_starting_priority + count.index
...
resource "aws_autoscaling_attachment" "attach" {
  count                  = var.using_server_group ? 0 : 1
