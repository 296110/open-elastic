...
variable "asg_name" {
  description = "The name of the ASG (ASG) in the servers are deployed. Leave this blank if using with a Server Group"
  type        = string
  default     = null
}

variable "using_server_group" {
  description = "Whether we're using an ASG or Gruntwork's Server Group module. If using ASG, then var.asg_name must be specified. Otherwise leave it blank"
  type        = bool
}
...
variable "http_listener_arns" {
  description = "The HTTP ARNs of ALB listeners to which Listener Rules that route to this Target Group should be added."
  type        = list(string)
  default     = []
}

variable "num_http_listener_arns" {
  description = "The number of HTTP ARNs in var.listener_arns. We should be able to compute this automatically, but due to a Terraform limitation, if there are any dynamic resources in var.listener_arns, then we won't be able to: https://github.com/hashicorp/terraform/pull/11482"
  type        = number
  default     = 0
}

variable "https_listener_arns" {
  description = "The HTTPS ARNs of ALB listeners to which Listener Rules that route to this Target Group should be added."
  type        = list(string)
  default     = []
}

variable "num_https_listener_arns" {
  description = "The number of HTTPS ARNs in var.listener_arns. We should be able to compute this automatically, but due to a Terraform limitation, if there are any dynamic resources in var.listener_arns, then we won't be able to: https://github.com/hashicorp/terraform/pull/11482"
  type        = number
  default     = 0
}
