...
tail -f --retry \
  ...
  "/var/log/elasticsearch/$USER_DATA_ENV_cluster_name.log" \
  2>/dev/null &

# We need systemd to run to fire up Elasticsearch itself. To run systemd, we have to run /sbin/init at the end of this
...
[Unit]
Description=Mock Run Elasticsearch Script
Documentation=https://github.com/gruntwork-io/terraform-aws-elk
[Service]
User=root
WorkingDirectory=/usr/share/elasticsearch/bin
